...
const { parseIntelligentWorkout } = require('./services/intelligentWorkoutParser'); // Not direct import, wait I'll use the class.

const IntelligentWorkoutParser = require('./services/intelligentWorkoutParser');
async function test() {
  const parser = new IntelligentWorkoutParser();
  // Override the model temporarily for this test
  const oldCreate = parser.groq.chat.completions.create.bind(parser.groq.chat.completions);
  parser.groq.chat.completions.create = async (args) => {
